...
const app = createApp({
    ...
    setup() {
        ...
        const columns = ref([
            ...
            { columnKey: 'customerGroupName', columnName: 'CustomerGroupName', columnCaption: 'Group', columnSort: 'CustomerGroup.Name' },
            { columnKey: 'customerSubGroupName', columnName: 'CustomerSubGroupName', columnCaption: 'Sub', columnSort: 'CustomerSubGroup.Name' },
        ...
        const {
            paged,
            apiUrl
        } = usePagedList('/Customer/GetPagedCustomer', ``)
        ...
        const customerSubGroupLookup = ref([])
            ...
            Object.assign(formData,
                {
                    ...
                    customerSubGroupId: null,
                    customerSubGroupName: null,
                    name: null,
                    code: null,
                    description: null,
                    street: null,
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    try {
                        ...
                        const response = await request('delete', `/Customer/DeleteCustomer`, {
        ...
        const formValidation = () => {
            ...
            if (!formData.customerGroupId) {
                errorData.customerGroupId = 'CustomerGroup is required.'
                valid = false
            }
            if (!formData.street) {
        ...
        const handleSubmitForm = async () => {
            ...
            try {
                const response = await request(
                    'post',
                    pageMode.create ? '/Customer/CreateCustomer' : '/Customer/UpdateCustomer',
                    {
                        ...
                        customerSubGroupId: formData.customerSubGroupId,
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        if (pageMode.create) {
                            ...
                            formData.customerSubGroupName = customerSubGroupLookup.value.find(customerSubGroup => customerSubGroup.value === formData.customerSubGroupId)?.text || 'N/A'
        ...
        const handleTableActionRowClick = async (payload) => {
            ...
            //load child
            pagedChild.data = payload.customerContacts
            pagedChild.selectedRow = null
        }


        watch(() => formData.customerGroupId, async (newValue, oldValue) => {
            let url = '/CustomerSubGroup/GetCustomerSubGroupLookup'
            if (newValue !== null) {
                url += `?customerGroupId=${newValue}`
            }
            const respCustomerSubGroupLookup = await request('get', url)
            customerSubGroupLookup.value = respCustomerSubGroupLookup?.data?.content?.data
        })
        ...
        const handleSubmitFormChild = async () => {
            ...
            try {
                ...
                if (childMode.create) {
                    url = '/CustomerContact/CreateCustomerContact'
                } else if (childMode.update) {
                    url = '/CustomerContact/UpdateCustomerContact'
                } else if (childMode.delete) {
                    url = '/CustomerContact/DeleteCustomerContact'
                    method = 'delete'
                }
                ...
                const response = await request(
                    ...
                    url,
                    {
                        ...
                        userId: userId.value,
                        customerId: paged.selectedRow.id,
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        await getDataTable()
                        pagedChild.data = paged.data.find(item => item.id === paged.selectedRow.id)?.customerContacts
        ...
        const preparePage = async () => {
            ...
            await getDataTable()
            const respCustomerGroupLookup = await request('get', '/CustomerGroup/GetCustomerGroupLookup', {})
            customerGroupLookup.value = respCustomerGroupLookup?.data?.content?.data
