...
const app = createApp({
    ...
    setup() {
        ...
        const columns = ref([
            ...
            { columnKey: 'vendorName', columnName: 'VendorName', columnCaption: 'VendorName', columnSort: 'Vendor.Name' }
        ])
        const {
            paged,
            apiUrl
        } = usePagedList('/VendorContact/GetPagedVendorContact', ``)
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    try {
                        ...
                        const response = await request('delete', `/VendorContact/DeleteVendorContact`, {
                            id: paged.selectedRow.id,
                            userId: userId.value,
                            vendorId: paged.selectedRow.vendorId
                        })
        ...
        const formValidation = () => {
            ...
            if (!formData.vendorId) {
                errorData.vendorId = 'Vendor is required.'
                valid = false
            }
        ...
        const handleSubmitForm = async () => {
            ...
            try {
                const response = await request(
                    'post',
                    pageMode.create ? '/VendorContact/CreateVendorContact' : '/VendorContact/UpdateVendorContact',
                    {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        if (pageMode.create) {
                            ...
                            formData.vendorName = vendorLookup.value.find(vendor => vendor.value === formData.vendorId)?.text || 'N/A'
        ...
        const preparePage = async () => {
            ...
            await getDataTable()
            const respVendorLookup = await request('get', '/Vendor/GetVendorLookup', {})
            vendorLookup.value = respVendorLookup?.data?.content?.data
